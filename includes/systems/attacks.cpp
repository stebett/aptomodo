...
b2BodyId createBody(const entt::entity entity) {
...
void Assign(const b2BodyId &attacker, const std::string &name) {
    ...
    Attacks::Transform transform {    Spline{trajPoints}    };
    transform.loadFromTOML(name);
    ...
    Game::registry.emplace<Attacks::BodyCouple>(bodyEntity, Attacks::BodyCouple{attacker, body});
    float duration {1}; // TODO add in the attack toml file
    ...
    if (Game::registry.
        ...
        Game::registry.
            emplace_or_replace<Attacks::Transform>(bodyEntity, transform
        );
